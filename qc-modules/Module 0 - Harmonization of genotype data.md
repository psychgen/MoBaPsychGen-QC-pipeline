...
| Batch | Genotyping array | Current genotyping build |
| --- | --- | --- |
| HARVEST12good (m12a) | HumanCoreExome12v1.1 | GRCh37 (hg19) |
| HARVEST12bad (m12b) | HumanCoreExome12v1.1 | GRCh37 (hg19) |
| HARVEST24 | HumanCoreExome24v1.0 | GRCh37 (hg19) |
| NORMENT-Jan-15 | OmniExpress24v1.0 | GRCh37 (hg19) |
| NORMENT-Jan-15 | OmniExpress24v1.0 | GRCh37 (hg19) |
| NORMENT-May-16 | OmniExpress24v1.2 | GRCh38 (hg38) |
| ADHD | OmniExpress24v1.2 | GRCh37 (hg19) |
| NORMENT-Feb-18 | Global Screening Array v.1.0 | GRCh38 (hg38) |
| Rotterdam1 | Global Screening Array v.1.0 | GRCh37 (hg19) |
| Rotterdam2 | Global Screening Array v.1.0 | GRCh37 (hg19) |
| ADHD2 | Global Screening Array v.1.0 | GRCh38 (hg38) |
| NORMENT-Feb-20-V1 | Global Screening Array v.1.0 | GRCh38 (hg38) |
| NORMENT-Feb-20-V3 | Global Screening Array v.3.0 | GRCh38 (hg38) |
| NORMENT-Aug-20-996 | Global Screening Array v.3.0 | GRCh38 (hg38) |
| NORMENT-Aug-20-1029 | Global Screening Array v.3.0 | GRCh38 (hg38) |
| NORMENT-Nov-20-1066 | Global Screening Array v.3.0 | GRCh38 (hg38) |
| NORMENT-Nov-20-1077 | Global Screening Array v.3.0 | GRCh38 (hg38) |
| NORMENT-Nov-20-1108 | Global Screening Array v.3.0 | GRCh38 (hg38) |
| NORMENT-Nov-20-1109 | Global Screening Array v.3.0 | GRCh38 (hg38) |
| NORMENT-Nov-20-1135 | Global Screening Array v.3.0 | GRCh38 (hg38) |
| NORMENT-Nov-20-1146 | Global Screening Array v.3.0 | GRCh38 (hg38) |
| NORMENT-Mar-21-1273 | Global Screening Array v.3.0 | GRCh38 (hg38) |
| NORMENT-Mar-21-1409 | Global Screening Array v.3.0 | GRCh38 (hg38) |
| NORMENT-Mar-21-1413 | Global Screening Array v.3.0 | GRCh38 (hg38) |
| NORMENT-Mar-21-1531 | Global Screening Array v.3.0 | GRCh38 (hg38) |
| NORMENT-Mar-21-1532 | Global Screening Array v.3.0 | GRCh38 (hg38) |
...
1.
## Initial batch files and information

  1. Copy the set of binary PLINK files (bed, bim and fam files) of the batch assigned to you from &quot;original\_data&quot; folder to your working directory (the one named with your initials). Never work on the original data directly!
  2. Record the number of SNPs in your batch
 wc -l original.bim
  3. Record the number of individuals in your batch
 wc -l original.fam
2.
## Update SNP alleles

  1. For the HumanCoreExome batches only: update SNP alleles (convert the Illumina A/B alleles to the genetic A,C,T,G alleles)
  2. Using the information on the following website - [https://www.well.ox.ac.uk/~wrayner/strand/ABtoTOPstrand.html](https://www.well.ox.ac.uk/~wrayner/strand/ABtoTOPstrand.html), in the &quot;resources&quot; folder in DATA/DURABLE there are the following files to use to update alleles:
 ...
 Copy the file corresponding to your batch to your working directory in DATA/DURABLE and record the total number of SNPs for which there is allele information.
  3. Run the following command to update the alleles:
 ...
 &quot;name.of.the.file.with.alleles.of.your.array&quot; is the name of the file that contains allele information for the genotype array corresponding to your batch.
  4. Record the number of number of SNPs whose alleles were updated
  5. Remove SNPs whose alleles were not updated using the following command:
 plink --bfile original-initials-alleles-update --exclude original-initials-alleles-update.allele.no.snp --make-bed --out original-initials-alleles
  6. Record the number of SNPs removed.
  7. Create a list of SNPs that were removed
...
When available, a list of known badly performing SNPs (black-listed SNPs) will be excluded from the data. There is such a list for HumanExome array and because it is rather similar to the content on the HumanCoreExome array, data genotypes on that array will be filtered for those badly performing SNPs. Because HARVEST is genotypes on HumanCoreExome array, we will removing black-listed SNPs from these data, using the steps below.
  ...
  1. Copy the list of badly performing SNPs from the &quot;resources&quot; folder to your working directory, the file is named pchip\_blackList\_dec2015\_stripped.txt.
  2. Exclude blacklisted SNPs using the following PLINK command:
 plink --bfile original-initials-alleles --exclude pchip\_blackList\_dec2015\_stripped.txt --make-bed --out original-initials-no-black-list
  3. Add the IDs of the removed SNPs to the original-initials-bad-snps.txt list.
...
## Update SNP names to rsIDs
 ...
 Copy the file that matches the chip version of your batch (this information is available in the table at the beginning of the document) to your working directory in DATA/DURABLE and record the total number of SNPs for which there rsID SNP information.
  3. Update the SNP names to correspond to the rsID using the below command:
 plink --bfile original --update-name name.of.the.file.with.rsIDs.of.your.chip --make-bed --out original-initials-rsids
2.
## Update the chromosomal positions to build GRCh37/hg19
