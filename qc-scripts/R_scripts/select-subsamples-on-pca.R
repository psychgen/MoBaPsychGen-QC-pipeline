...
# Function: This script selects core (e.g. European, African, Asian) subsamples based on PCA plot(s) of the study data with 1KG, generates zoom and threshold plots based on given thresholds, and generates a text file containing the list of selected individuals per core subsample.
...
# Arguments:  datalabel - a compact name of the study data to be used as a label
...
# Example:    Rscript select-subsamples-on-pca.R Norment_jun2015 norment_batch4_jun2015-1kg-yc-pca norment_batch4_jun2015-yc norment-jun2015-yc-pca-core-select-custom.txt
...
data_zoom_list <- list()
data_threshold_list <- list()
abline_text_list <- list()
...
if (length(tag_list) > 0) {
    ...
    for (i in 1:length(custom)) {
        ...
        }else if (grepl("draw_threshold", custom[i], fixed = TRUE)==TRUE) {
            threshold_text=paste0("data_threshold <- data[", line ,", ]")
            eval(parse(text=threshold_text))
            data_threshold_list <- append(data_threshold_list,list(data_threshold))
            abline_text = ""
            for (l in unlist(strsplit(line, "&"))) {
                l = gsub(">", "", l)
                l = gsub("<", "", l)
                if (grepl("V3", l, fixed = TRUE)==TRUE) {
                    l = gsub("data\\$V3", "", l)
                    l = gsub("^\\s+|\\s+$", "", l)
                    abline_text = paste0(abline_text, "abline(v=",l,", col='red'); ")
                } else if (grepl("V4", l, fixed = TRUE)==TRUE) {
                    l = gsub("data\\$V4", "", l)
                    l = gsub("^\\s+|\\s+$", "", l)
                    abline_text = paste0(abline_text, "abline(h=",l,", col='red'); ")
                }
            }
            abline_text_list <- append(abline_text_list,abline_text)
...
if (length(tag_list) > 0) {
    for (i in 1:length(tag_list)) {
        ...
        if (length(data_threshold_list) > 0) {
            data_threshold <- data_threshold_list[[i]]
            abline_text <- abline_text_list[[i]]
        ...
        plot(data_zoom[,3],data_zoom[,4],pch=20,main=paste0(datalabel," + 1KG zoomed ",toupper(tag),", PC1 vs PC2"),col=data_zoom$color,xlab="PC1",ylab="PC2")
        legend(legend_position, title=paste0(toupper(tag),' subsample'), legend=paste0(df$Var1,' (',df$Freq,')'), fill=unique(data_threshold$color))
        if (length(data_threshold_list) > 0) {
            eval(parse(text=abline_text))
        }

        data_zoom2=rbind(data_zoom[data_zoom$label!=datatag,],data_zoom[data_zoom$label==datatag,])
        plot(data_zoom2[,3],data_zoom2[,4],pch=20,main=paste0(datalabel," + 1KG zoomed ",toupper(tag),", PC1 vs PC2"),col=data_zoom2$color,xlab="PC1",ylab="PC2")
        legend(legend_position, title=paste0(toupper(tag),' subsample'), legend=paste0(df$Var1,' (',df$Freq,')'), fill=unique(data_threshold$color))
        if (length(data_threshold_list) > 0) {
            eval(parse(text=abline_text))
        }
        ...
        par(mfrow=c(2,3))
        plot(data_threshold[,3],data_threshold[,4],pch=20,main=paste0(datalabel," + 1KG threshold ",toupper(tag),", PC1 vs PC2"),col=data_threshold$color,xlab="PC1",ylab="PC2")
